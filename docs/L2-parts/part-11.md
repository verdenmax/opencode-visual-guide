...
- **复用全书机制**：L58「trigger 传草稿、插件涂改」呼应 L54 `produce`、L47 IoC「编辑一份草稿」；L59 复用语言服务器、L60 复用 PTY 生态、L61 复用 git，同 L39 Ripgrep/L51 git「复用成熟工具」；L59 诊断「只报最关键」呼应 L42/L59 注意力稀缺；L60 双后端同接口呼应 L52 渲染器可插拔；L60 的 env 复用 L58 `shell.env` 钩子（积木相扣）；L61 Location 作用域注入呼应 M2 Effect DI；L61「解耦是什么与在哪里」呼应 L47/L50/L54/L56「找准接缝」。
