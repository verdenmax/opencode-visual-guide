...
| 课 | 主题 | 一句话 |
| --- | --- | --- |
| L09 | server 总览 | 不是 Hono/Express，而是 Effect 的 HttpApi；对外是一个 webHandler `(request)=>Response`；20 个路由组（21 个 .ts 文件）；`OpenApi.fromApi`→SDK |
| L10 | 路由组与 handler | 组=声明契约(纯类型)、handler=接到 core(薄)、中间件=外圈横切关卡；20 组 + 20 handler + 9 中间件 |
