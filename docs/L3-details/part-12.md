...
- **monorepo 结构**：`packages/opencode`（核心+server）、`cli/tui/`（即 `src/cli/tui/`，TUI，SolidJS+opentui）、`packages/app`（共享 Web UI）、`packages/desktop`（Electron 包 app）、`packages/plugin`（`@opencode-ai/plugin` 源）；改 server/SDK 后跑 `script/generate.ts` 重生成 SDK。
