...
| --- | --- | --- |
| `packages/tui/src/app.tsx` | TUI 入口 `run(input)`：`Effect.acquireRelease(createCliRenderer({externalOutputMode:"passthrough",targetFps:60,exitOnCtrlC:false,useKittyKeyboard:{},useMouse,consoleOptions}), r=>destroyRenderer(r))`；`render(()=><ExitProvider…<App/>…>)` 挂组件树；二十多层 context provider 嵌套；SIGHUP→destroyRenderer | L52, L53 |
...
| `packages/tui/src/context/data.tsx` | `DataProvider`/`useData`：经 `useEvent()`（内部 `useSDK()`）独立消费 SDK 事件流（与 SyncProvider 并行、彼此不依赖），进一步派生/归约的响应式数据 | L53, L54 |
