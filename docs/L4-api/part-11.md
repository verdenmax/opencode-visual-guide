...
| `packages/opencode/src/agent/agent.ts:379` | trigger `system.transform`：系统提示成形处，插件可改 system prompt（experimental.chat.system.transform） | L58 |
...
| `packages/opencode/src/acp/agent.ts` | `Agent` 实现 ACP 的 `ACPAgent`：initialize/authenticate/newSession/loadSession/forkSession/prompt/cancel/setSessionModel·Mode·ConfigOption（`forkSession`/`setSessionModel` 在 ACP 接口上以 `unstable_` 前缀暴露，委托给同名 service 方法）——opencode 当 **ACP 服务器**被编辑器(Zed)消费，是 L59 LSP 客户端的镜像 | L61 |
