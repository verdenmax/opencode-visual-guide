...
<p>下面把这 11 跳<strong>一次性铺平</strong>，并标出每跳属于下行（<span class="mono">↓</span> 收拢请求）、中段循环（<span class="mono">↻</span>）还是上行（<span class="mono">↑</span> 广播事件）：</p>
<div class="trace">
  <div class="t-row"><span class="t-num">①↓</span><span class="t-txt">按键落进 TUI prompt 组件（<span class="mono">packages/tui/src/component/prompt</span>）</span></div>
  <div class="t-row"><span class="t-num">②↓</span><span class="t-txt"><span class="mono">createWorkerFetch</span> 把 SDK.fetch 偷换成对 Worker 的 RPC（<span class="mono">cli/cmd/tui.ts</span>）</span></div>
  <div class="t-row"><span class="t-num">③↓</span><span class="t-txt">worker 里的 <span class="mono">Server.listen</span> → Effect <span class="mono">HttpApi</span> 的 session 路由组</span></div>
  <div class="t-row"><span class="t-num">④↓</span><span class="t-txt">prompt 落成事件溯源的收件箱记录、唤醒运行器（<span class="mono">session/input.ts</span> → <span class="mono">run-coordinator.ts</span>）</span></div>
  <div class="t-row"><span class="t-num">⑤↓</span><span class="t-txt">渲染基线 System Context（<span class="mono">system-context/index.ts</span>）</span></div>
  <div class="t-row"><span class="t-num">⑥↓</span><span class="t-txt">投影历史 → LLM 消息、解析模型（<span class="mono">runner/to-llm-message.ts</span> · <span class="mono">model.ts</span>）</span></div>
  <div class="t-row"><span class="t-num">⑦↓</span><span class="t-txt"><span class="mono">llm.stream()</span> 经协议适配器发给 provider，拿回 <span class="mono">Stream&lt;LLMEvent&gt;</span></span></div>
  <div class="t-row"><span class="t-num">⑧↻</span><span class="t-txt"><strong>循环·问</strong>：一次 provider turn</span></div>
  <div class="t-row"><span class="t-num">⑨↻</span><span class="t-txt"><strong>循环·干</strong>：tool-call → <span class="mono">FiberSet</span> 并发 settle → 持久化结果</span></div>
  <div class="t-row"><span class="t-num">⑩↻</span><span class="t-txt"><strong>循环·判</strong>：还有工具调用？是 → 回 ⑧；否 → 收尾（护栏 <span class="mono">MAX_STEPS=25</span>）</span></div>
  <div class="t-row"><span class="t-num">⑪↑</span><span class="t-txt">增量持久化发事件 → <span class="mono">GlobalBus</span> → SSE/RPC → 16ms 批渲染 → 屏幕</span></div>
</div>
...
<p>Here are all 11 hops <strong>laid flat in one view</strong>, each marked as down-pass (<span class="mono">↓</span> converging the request), mid-loop (<span class="mono">↻</span>), or up-pass (<span class="mono">↑</span> broadcasting events):</p>
<div class="trace">
  <div class="t-row"><span class="t-num">①↓</span><span class="t-txt">keystroke lands in the TUI prompt component (<span class="mono">packages/tui/src/component/prompt</span>)</span></div>
  <div class="t-row"><span class="t-num">②↓</span><span class="t-txt"><span class="mono">createWorkerFetch</span> swaps SDK.fetch for an RPC to the Worker (<span class="mono">cli/cmd/tui.ts</span>)</span></div>
  <div class="t-row"><span class="t-num">③↓</span><span class="t-txt">the worker's <span class="mono">Server.listen</span> → Effect <span class="mono">HttpApi</span>'s session route group</span></div>
  <div class="t-row"><span class="t-num">④↓</span><span class="t-txt">prompt lands as an event-sourced inbox record, wakes the runner (<span class="mono">session/input.ts</span> → <span class="mono">run-coordinator.ts</span>)</span></div>
  <div class="t-row"><span class="t-num">⑤↓</span><span class="t-txt">render the baseline System Context (<span class="mono">system-context/index.ts</span>)</span></div>
  <div class="t-row"><span class="t-num">⑥↓</span><span class="t-txt">project history → LLM messages, resolve the model (<span class="mono">runner/to-llm-message.ts</span> · <span class="mono">model.ts</span>)</span></div>
  <div class="t-row"><span class="t-num">⑦↓</span><span class="t-txt"><span class="mono">llm.stream()</span> goes through the protocol adapter to the provider, returning <span class="mono">Stream&lt;LLMEvent&gt;</span></span></div>
  <div class="t-row"><span class="t-num">⑧↻</span><span class="t-txt"><strong>loop · ask</strong>: one provider turn</span></div>
  <div class="t-row"><span class="t-num">⑨↻</span><span class="t-txt"><strong>loop · work</strong>: tool-call → <span class="mono">FiberSet</span> concurrent settle → persist results</span></div>
  <div class="t-row"><span class="t-num">⑩↻</span><span class="t-txt"><strong>loop · decide</strong>: more tool calls? yes → back to ⑧; no → wind down (guardrail <span class="mono">MAX_STEPS=25</span>)</span></div>
  <div class="t-row"><span class="t-num">⑪↑</span><span class="t-txt">incremental persistence emits events → <span class="mono">GlobalBus</span> → SSE/RPC → 16ms batch render → screen</span></div>
</div>
...
<p>It's worth pausing here: why is hop ⑦ a "stream" rather than a "one-shot return"? Because a large model generates <strong>token by token</strong>; waiting for it to finish the whole passage before returning would leave you idling for a dozen seconds with nothing on screen. Streaming lets the server, the instant it grabs a fragment, <strong>receive, persist, and emit events all at once</strong>, so you see the first character pop out almost immediately. More crucially, <strong>tool calls hide inside this stream too</strong>—the model might write half a sentence, then suddenly slip in "I want to call the edit tool," and the runner must pull it out and execute it <strong>mid-stream</strong>, not wait for the whole passage to finish. So this hop's true posture isn't "ask, wait, get the answer," but "<strong>listen, record, and stay ready to act, all at once</strong>." Think this through and you'll grasp why V2's runner isn't a simple request/response function but a state machine revolving around an event stream.</p>
...
<p>The trouble with boulder files is far more than "tiring to read." When calling the model, processing the stream, running tools, generating titles, and compacting history are all crammed into a 1,700-line cluster of functions, any single change can <strong>pull one hair and move the whole body</strong>, and testing one small piece in isolation is nearly impossible—their states are tangled together and you can't lift out any segment to verify cleanly. Once V2 splits each concern into an independent small collaborator, you can <strong>feed input and verify output in isolation</strong>, changing one spot without dreading a global tremor. That is the most concrete dividend "splitting" buys: <strong>testable, replaceable, independently evolvable</strong>. The more critical the code, the more this controllability is worth—and the session engine is exactly opencode's most critical piece.</p>
