...
<p>布局也是你熟悉的那套：opencode 的 TUI 代码里到处是 <span class="mono">flexDirection</span>（122 处）、<span class="mono">flexGrow</span>、<span class="mono">justifyContent</span>、<span class="mono">alignItems</span>——<strong>就是 CSS flexbox</strong>。opentui 内置了一个 flexbox 布局引擎，按这些属性算出每个元素该占终端的哪几行哪几列、多宽多高。于是「左边固定宽的侧边栏 + 右边自适应的主区」这种布局，你用 <span class="mono">flexDirection:"row"</span> + 侧边栏定宽 + 主区 <span class="mono">flexGrow:1</span> 就搞定了，和写网页<strong>一字不差</strong>。</p>
<p>这种「把成熟模型整体复用」的思路，正是全书反复出现的智慧（L51 复用 git、L39 复用 Ripgrep）——只不过这次复用的，是整个<strong>前端渲染范式</strong>。值得想一想这背后省掉了多少事：终端绘图的传统做法，是直接拼接 ANSI 转义码——「把光标移到第 5 行第 12 列、设成红色、打印这串字、再复位」。</p>
<p>这种手工绘图既<strong>难写又难维护</strong>：稍微复杂一点的布局（一个会随窗口缩放的多栏界面）就得自己算每个字符的坐标，改一处动全身。而 opentui 把这层彻底<strong>抽象掉了</strong>：你只声明「我要一个横向排列、左栏定宽、右栏自适应的盒子」，至于「第 5 行第 12 列该打什么字、什么颜色」，全交给布局引擎和绘制器去算。<strong>从「手动摆字符」到「声明式描述布局」，正是 TUI 开发体验的一次代际跃迁</strong>——而这一跃，opencode 没有自己造，是借 opentui 白捡的。</p>
...
<p>这个嵌套顺序<strong>绝非随意</strong>——它是一张<strong>依赖关系图</strong>的拓扑排序。看 <span class="mono">project.tsx</span> 就明白了：<span class="mono">ProjectProvider</span> 的 <span class="mono">init</span> 函数第一行就是 <span class="mono">const sdk = useSDK()</span>——它要用 SDK 去拉项目信息。既然 <span class="mono">useSDK()</span> 只能在 <span class="mono">SDKProvider</span> 内部用，那 <span class="mono">ProjectProvider</span> 就<strong>必须</strong>被 <span class="mono">SDKProvider</span> 包在里头。</p>
<p>同理，<span class="mono">RouteProvider</span> 的 <span class="mono">init</span> 只用 <span class="mono">useTuiStartup()</span>、<strong>并不</strong>依赖 SDK，所以它<strong>反而裹在 <span class="mono">SDKProvider</span> 之外</strong>（同在外层的还有 Toast、KV 等）；而 <span class="mono">SyncProvider</span> 与 <span class="mono">DataProvider</span> 的 <span class="mono">init</span> 都通过 <span class="mono">useEvent()</span>（其内部又调 <span class="mono">useSDK()</span>）<strong>各自独立地</strong>消费 SDK 的事件流——两者都依赖 SDK、故都在 SDKProvider 之内（它俩彼此并不依赖，<span class="mono">Sync→Data</span> 的相邻只是惯例排序）……<strong>每一处「内层 init 调用外层的 use」，都在这座金字塔里钉下一条「谁必须在谁外面」的硬约束</strong>。于是这座塔从下到上，恰好是一条「越基础越靠外、越上层越靠内」的依赖链：连接（SDK）→ 项目 → 数据同步 → 主题 → 交互状态 → 界面。</p>
<p>这种「用嵌套顺序表达依赖」的手法，其实比一份显式的依赖声明更巧妙：你<strong>无需另写一张「谁依赖谁」的表</strong>，依赖关系就<strong>物理地</strong>体现在代码的缩进层级里——外层一定先于内层就位，内层天然能用上外层的一切。读 <span class="mono">app.tsx</span> 那段嵌套，从上往下读一遍，就等于把整个 App 的「服务依赖图」从地基到屋顶走了一遍。一个新人想搞懂「TUI 启动时各种服务按什么顺序就位」，不必翻文档，看这一段 JSX 的嵌套顺序即可——<strong>结构本身就是文档</strong>。</p>
...
<p>这个公式的妙处，在于它<strong>同时打败了两种朴素方案</strong>。如果<strong>只按频率排</strong>（开得多的在前），那你半年前因为某个原因疯狂打开过几十次的文件，会<strong>永远霸占榜首</strong>——哪怕你早就不碰它了。如果<strong>只按最近排</strong>（刚开的在前），那你<strong>误点一次</strong>的某个无关文件，会瞬间插到你天天用的核心文件前面。</p>
<p>而 frecency 用「频率<strong>除以</strong>时间衰减」把两者揉到一起：一个文件哪怕历史频率很高，只要久不碰，分母 <span class="mono">(1+天数)</span> 就把它的分数<strong>慢慢拖低</strong>；而一个刚开的新文件，频率虽低但 <span class="mono">天数≈0</span>、分母≈1，能暂时靠前——但若你不再回头开它，它的分也很快随频率不再增长而被后来者超过。<strong>「频率给你长期偏好，最近度给你当下意图，相除让两者动态平衡」</strong>——这就是为什么你敲 <span class="mono">@</span> 时，列表顶端总是「你这阵子又常用、又刚碰过」的那几个文件。</p>
<p>这个算法不是 opencode 的独创，而是 Firefox 地址栏、各种编辑器文件选择器背后<strong>同一个久经验证的排序智慧</strong>——一个值得你装进工具箱、随处可用的小而美的算法。三种排序方案的高下，并排一看便知：</p>
<div class="cols">
...
<span class="kw">function</span> <span class="fn">updateFrecency</span>(filePath) {
  ...
  <p>最值得品的，是这个设计如何用<strong>一个极简的数学式</strong>，优雅地表达了一个本来很「主观」的需求——「把我最可能想要的文件排前面」。「我最可能想要什么」听起来像要靠机器学习、要建用户画像的复杂问题，但 frecency 用一行除法就给出了一个<strong>足够好</strong>的答案：你的<strong>长期偏好</strong>（频率）和<strong>当下意图</strong>（最近度）本就是预测「你想要什么」的两个最强信号，而「频率 ÷ 时间」恰好让它们以一种<strong>会自动新陈代谢</strong>的方式融合——新习惯随你的打开行为不断累积上分，旧习惯随时间流逝自动褪色。</p>
<p><strong>不需要任何显式的「过期」「清理」逻辑，时间衰减让排序永远自动跟着你「最近这阵子」的真实状态走。</strong>这是一个绝佳的范例，说明<strong>很多看似需要「智能」的产品体验，其实一个想透了的简单公式就能漂亮地解决</strong>——关键不在算法多复杂，而在你是否看清了「真正该度量的是什么」。</p>
<p>把它装进你的工具箱：任何「最近常用项排序」的需求，frecency 都是第一个该想到的答案。</p>
</div>
...
<p>This formula's beauty is that it <strong>defeats two naive schemes at once</strong>. If you <strong>rank by frequency only</strong> (most-opened first), a file you frantically opened dozens of times half a year ago for some reason would <strong>forever squat at the top</strong>—even if you stopped touching it long ago. If you <strong>rank by recency only</strong> (just-opened first), some irrelevant file you <strong>mis-clicked once</strong> would instantly jump ahead of the core file you use daily.</p>
<p>frecency kneads both together via "frequency <strong>divided by</strong> time decay": however high a file's historical frequency, as long as it's untouched a while, the denominator <span class="mono">(1+days)</span> <strong>slowly drags its score down</strong>; while a just-opened new file, though low-frequency, has <span class="mono">days≈0</span> and denominator≈1, can rank up temporarily—but if you don't reopen it, its score is soon overtaken as its frequency stops growing. <strong>"Frequency gives you long-term preference, recency gives you present intent, division dynamically balances the two"</strong>—that's why, when you type <span class="mono">@</span>, the list's top is always the few files you've "lately both used often and just touched."</p>
<p>This algorithm isn't opencode's invention but the <strong>same time-tested ranking wisdom</strong> behind Firefox's address bar and various editors' file pickers—a small, beautiful algorithm worth putting in your toolbox for use anywhere. The merits of the three ranking schemes, side by side, are clear at a glance:</p>
<div class="cols">
