...
<span class="kw">export function</span> <span class="fn">report</span>(file, issues) {
  ...
  <p>这二十来行代码里，藏着一个常被新手忽略、却对 agent 体验至关重要的判断：<strong>什么该说、什么不该说。</strong></p>
<p>一个朴素的实现可能会把语言服务器吐出的<strong>所有</strong>诊断（错误、警告、提示、风格建议）一股脑塞给模型——结果是灾难：模型的上下文被几百条「这里少个分号」「建议用 const」的噪音淹没，真正会导致编译失败的那三条 ERROR 反而被埋了。<span class="mono">report</span> 做了三层克制：<strong>只报 ERROR</strong>（agent 当下最该管的是「能不能编译过」，warning 多是风格偏好，可以稍后再说）、<strong>每文件封顶 20 条</strong>（一个文件错到 20 条以上，再多列也没意义，先修这些）、<strong>没错误就返回空字符串</strong>（一个字都不浪费）。</p>
<p>这背后是一个深刻的产品直觉：<strong>给 agent 的反馈，不是越多越好，而是越「准」越好。</strong>信息过载和信息缺失一样有害——前者淹没重点、烧光上下文，后者让 agent 盲目。<span class="mono">report</span> 这二十行的全部用心，就是在这两者之间，为「agent 此刻最需要知道什么」找到那个恰到好处的量。这和 L42 有界工具输出是同一种对「模型注意力稀缺」的敬畏，只不过这次稀缺的对象，是那几条<strong>真正要命</strong>的编译错误。</p>
</div>
...
<span class="kw">const</span> layer = (ref: Ref) =&gt; Layer.<span class="fn">effect</span>(Service, Effect.<span class="fn">gen</span>(<span class="kw">function*</span>() {
  ...
  <p>这个类小得不能再小，但它在 opencode 架构里的位置极其关键——<strong>它是一条「接缝」，把一整个会话从「它在哪儿」这件事上剥离了开来。</strong>注意它被实现成一个 Effect 的 <span class="mono">Service</span>（第 2 部分的依赖注入）：工具、权限、文件系统这些子系统，都<strong>通过依赖这个 <span class="mono">Location.Service</span> 来确定自己的作用域</strong>，而非各自去摸一个全局的「当前目录」。</p>
<p>这一个小小的设计选择，带来了惊人的杠杆：<strong>因为「位置」是被注入的、而非写死的，所以换一份 Location，整个会话的「文件系统视图、权限边界、能跑的命令」就整体平移到了新位置——而会话的逻辑代码一行都不用改。</strong>这正是依赖注入（M2）最深刻的威力：把一个横切一切的关注点（「在哪儿」）抽成一个可注入的服务，于是「换地方」从「改一大堆硬编码路径」的噩梦，变成了「换一个注入值」的举手之劳。</p>
<p><span class="mono">workspaceID</span> 那个「为未来集群/远程放置预留」的可选字段，更透露了这条接缝的雄心：今天它让会话能在本地不同目录间搬家，明天它就能让会话在<strong>不同机器、不同远程位置</strong>间流转。<strong>一个想透了的底层抽象，不只解决眼前的问题，还为还没到来的未来，悄悄留好了门。</strong></p>
</div>
