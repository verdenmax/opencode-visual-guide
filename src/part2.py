...
<span class="kw">if</span> (Cause.<span class="fn">hasInterrupts</span>(cause)) <span class="kw">yield</span>* FiberSet.<span class="fn">clear</span>(toolFibers)</pre>
  这段 import 行就写着 <span class="mono">FiberSet, Semaphore, Cause, Stream</span>——一行并发原语清单。
</div>
...
<span class="kw">if</span> (Cause.<span class="fn">hasInterrupts</span>(cause)) <span class="kw">yield</span>* FiberSet.<span class="fn">clear</span>(toolFibers)</pre>
  Its import line literally reads <span class="mono">FiberSet, Semaphore, Cause, Stream</span> — a one-line concurrency-primitive inventory.
</div>
