...
<p>顺带点破一个结构：这些组并非平铺成一坨，而是<strong>分三层装配</strong>——<span class="mono">RootHttpApi</span>（控制面/全局）、<span class="mono">InstanceHttpApi</span>（每个工作区实例自己的那批组）、外加 <span class="mono">EventApi</span>，最后合成一个 <span class="mono">OpenCodeHttpApi</span>（见 <span class="mono">routes/instance/httpapi/api.ts</span>）。<span class="mono">routes/instance/</span> 这个路径本身就在喊话：路由是<strong>按实例</strong>组织的——这正是第 11 课「按实例/工作区过滤事件」的结构由来。</p>
<div class="layers">
...
<p>One structural note worth surfacing: these groups aren't a flat blob but are assembled in <strong>three tiers</strong>—<span class="mono">RootHttpApi</span> (control-plane/global), <span class="mono">InstanceHttpApi</span> (each workspace instance's own groups), plus <span class="mono">EventApi</span>, finally composed into one <span class="mono">OpenCodeHttpApi</span> (see <span class="mono">routes/instance/httpapi/api.ts</span>). The <span class="mono">routes/instance/</span> path itself is a hint: routing is organized <strong>per instance</strong>—which is exactly where Lesson 11's "per-instance/workspace event filtering" comes from.</p>
<div class="layers">
...
<p>这里值得点明一处容易混淆的地方：你在本课看到的这些组、handler 和那 9 个中间件（<span class="mono">session</span>、<span class="mono">config</span>、<span class="mono">error.ts</span>…），其实就<strong>住在本仓库</strong>的 <span class="mono">packages/opencode/src/server/routes/instance/httpapi/</span> 下（<span class="mono">groups/</span>、<span class="mono">handlers/</span>、<span class="mono">middleware/</span>），由同目录的 <span class="mono">api.ts</span> 装配成 Root / Instance / OpenCode 三层 API。</p>
<p><strong>另外</strong>还有一个更小的可复用包 <span class="mono">@opencode-ai/server</span>（<span class="mono">packages/server/src</span>），导出它自己的一套基础组（agent、message、model、fs、skill…）；<span class="mono">api.ts</span> 用 <span class="mono">.addHttpApi(Api)</span> 把它也合并进来。所以它是「又一块拼图」，而非「契约的真身」——别被「两个 server」绕晕。</p>
...
<p>One easily-confused point worth naming here: the groups, handlers, and 9 middleware you see in this lesson (<span class="mono">session</span>, <span class="mono">config</span>, <span class="mono">error.ts</span>…) <strong>live right in this repo</strong>, under <span class="mono">packages/opencode/src/server/routes/instance/httpapi/</span> (<span class="mono">groups/</span>, <span class="mono">handlers/</span>, <span class="mono">middleware/</span>), assembled by the sibling <span class="mono">api.ts</span> into the Root / Instance / OpenCode three-tier API.</p>
<p>There is <strong>also</strong> a smaller reusable package <span class="mono">@opencode-ai/server</span> (<span class="mono">packages/server/src</span>) exporting its own base groups (agent, message, model, fs, skill…); <span class="mono">api.ts</span> merges it in too via <span class="mono">.addHttpApi(Api)</span>. So it's "one more puzzle piece," not "the contract's true home"—don't get lost in "two servers."</p>
