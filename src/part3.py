...
<p>这里值得点明一处容易混淆的地方：这些路由组与 handler 的<strong>契约表面</strong>，其实住在一个<strong>独立的包 <span class="mono">@opencode-ai/server</span></strong>（<span class="mono">packages/server/src</span>，含 <span class="mono">api/groups/handlers/middleware</span>）里；而第 9 课讲的 <span class="mono">packages/opencode/src/server</span>，则负责把这套契约<strong>装配、启动</strong>成一个能跑的本地服务器（<span class="mono">webHandler</span>、MDNS、WebSocket 那些）。一个定义「API 长什么样」、一个负责「把它跑起来」——读源码时别被「两个 server」绕晕。</p>
...
<p>One easily-confused point worth naming here: these route groups and handlers—the <strong>contract surface</strong>—actually live in a <strong>separate package <span class="mono">@opencode-ai/server</span></strong> (<span class="mono">packages/server/src</span>, containing <span class="mono">api/groups/handlers/middleware</span>); while Lesson 9's <span class="mono">packages/opencode/src/server</span> is responsible for <strong>assembling and booting</strong> this contract into a runnable local server (the <span class="mono">webHandler</span>, MDNS, WebSocket, etc.). One defines "what the API looks like," the other "runs it"—reading the source, don't get lost in "two servers."</p>
