...
<p>把这件事和别的聊天工具对比一下就更清楚了。很多简单的对话应用，一条 AI 消息就是一个字符串字段——思考、工具、文字全糊成一团纯文本，想把工具调用单独画成卡片？做不到，因为信息在存的时候就被压扁了。</p>
<p>opencode 反其道而行：<strong>在最源头就把一条回复拆成带类型的零件</strong>，于是「能不能漂亮地渲染」「能不能实时更新某一块」这些问题，在数据进库的那一刻就已经有了肯定的答案。这又是那个反复出现的主题——<strong>形状定得好，下游全都顺；形状压扁了，再多的前端努力也补不回丢掉的结构</strong>。</p>
<p>reasoning 和 text 分开存也是同理：它让客户端可以选择「默认折叠思考、只显示答案」，而这只有在两者本就是不同 part 时才办得到。</p>
<div class="layers">
...
<p>Contrast this with other chat tools and it's clearer. Many simple conversation apps make an AI message one string field — thinking, tools, text all mashed into plain text; want to draw the tool call as its own card? Can't, because the information was flattened at storage time.</p>
<p>opencode does the opposite: <strong>it splits a reply into typed pieces right at the source</strong>, so questions like "can this render nicely" and "can a single block update in real time" already have a yes the moment data enters the store. This is that recurring theme again — <strong>get the shape right and everything downstream flows; flatten the shape and no amount of frontend effort recovers the lost structure</strong>.</p>
<p>Storing reasoning and text apart is the same logic: it lets clients choose "fold thinking by default, show only the answer," which is only possible when the two are distinct parts to begin with.</p>
<div class="layers">
...
<p>Walk these three turns and all the abstract parts come alive. In <strong>turn 1</strong>, the model is still "in the dark," and all it can do is <strong>initiate observation</strong> (read) — this turn produces almost no visible text, all in "pulling files."</p>
<p><strong>Turn 2</strong> is the crux: when the loop <strong>rereads history</strong>, it reads in the file content that turn 1's read brought back too — so this turn the model "can see" what the function looks like, only then can it talk of refactoring. Were it not "reread each round" but hoarding state in memory, this loop of "feeding the previous turn's tool result into the next turn's thinking" wouldn't close.</p>
<p>In <strong>turn 3</strong>, the work is done, the model calls no more tools and just gives a summary, <span class="mono">needsContinuation</span> flips to <span class="mono">false</span>, and the loop cleanly stops.</p>
...
<p>这套「写一种、读一种」的分家，在工程上有个名字叫 <strong>CQRS / 事件溯源读模型</strong>——但名字不重要，重要的是那个直觉：<strong>底账与报表，是同一份真相的两张脸。</strong>报表丢了不要紧，照底账能重新投影出来；但底账要是被改写了，那就真的失去了真相。</p>
<p>所以 opencode 把「不可变的权威」放在事件这一端，把「可重建的便利」放在投影这一端——<strong>权威的那份永不可变，好读的那份随时可弃可重建</strong>。这个主次一旦摆正，可靠和好读就不再打架了。</p>
<p>一旦你习惯用「底账 vs 报表」这副眼镜看系统，你会在很多地方认出它：数据库的物化视图、缓存与源数据、甚至 git 的提交历史与工作区——都是同一种「权威只追加、视图可重建」的分工。</p>
<div class="flow">
...
<p>This "write one shape, read another" split has an engineering name: <strong>CQRS / event-sourcing read model</strong> — but the name doesn't matter; what matters is the intuition: <strong>the ledger and the statement are two faces of one truth.</strong> Lose the statement and it's fine, it can be re-projected from the ledger; but rewrite the ledger and you've truly lost the truth.</p>
<p>So opencode puts "the immutable authority" at the event end and "the rebuildable convenience" at the projection end — <strong>the authoritative copy is forever immutable, the readable copy is discardable and rebuildable anytime</strong>. Once this priority is set right, reliability and readability stop fighting.</p>
<p>Once you get used to viewing systems through the "ledger vs statement" lens, you'll recognize it everywhere: a database's materialized views, a cache vs its source data, even git's commit history vs the working tree — all the same division of "authority only appends, the view rebuilds."</p>
<div class="flow">
...
<p>这里有个容易被忽略的分寸：上限是 25，不是 5、也不是 250。太小，复杂任务还没展开就被掐断，agent 显得「不中用」；太大，失控的代价（钱、时间、对你文件的折腾）又太可怕。</p>
<p>但要诚实地说——<strong>25 这个具体数字，源码自己也没把握</strong>：常量上方就挂着一行注释「<span class="mono">QUESTION: 这个限制是本来就有、还是我们后加的？它合理吗？</span>」。所以与其说它是精确调校出的最优值，不如说是一个<strong>「先定个数、别让它失控」的硬上限</strong>——它给了模型足够长的自驱空间去完成多数真实任务，又给了一道在它跑偏时及时止损的硬线。</p>
<p>安全护栏的艺术，往往不在「有没有」，而在「卡在哪个值」：太松等于没有，太紧等于碍事，而这个值<strong>本就允许日后再校准</strong>。正因如此，把这个数字钉成一个<strong>显式的常量</strong>（<span class="mono">MAX_STEPS</span>）而非散落在代码里的魔法数，才尤其要紧——它方便日后被审视、被调整，这又是这套代码「把关键决策（连同它的不确定）摆到明面上」的一贯做派。</p>
<div class="cols">
