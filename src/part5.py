...
<h2>安全边界：何时注入</h2>
<p>还有一个同样关键、却容易被忽略的问题：上下文变了，这条更新是<strong>什么时候</strong>被塞进历史的？你可能直觉地以为「源一变就立刻推」——但 opencode 偏偏<strong>不这么做</strong>。它的规矩是：<strong>变化被「懒惰地」采样、只在「安全的 provider-turn 边界」上才被采纳，绝不在源变化的当下异步推送。</strong>所谓<strong>安全 provider-turn 边界</strong>，源码的定义很精确：<strong>就在一次 provider 调用之前、且在「持久输入已提单、必要的工具已结清」之后</strong>的那个点。</p>
<div class="trace">
