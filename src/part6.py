...
<p>停下来体会「一种规范语言」这件事的分量。它的本质，是在 core 和外部世界之间，<strong>钉死一份双方都认的契约</strong>：core 承诺「我永远按这个形状提需求、按这个形状收响应」，适配器承诺「无论外面多乱，我都把它整理成这个形状」。</p>
<p>一旦这份契约立住，两边就能<strong>各自独立地演化</strong>：core 想加一种新的工具调用方式，只需在规范里加一笔，不必管十几家供应商怎么实现；某家供应商改了 API，只需改它那个适配器，core 浑然不觉。<strong>规范语言不是一种限制，而是一种解耦的自由</strong>——它让「业务怎么演化」和「供应商怎么演化」这两件本会互相拖累的事，彻底分了家。</p>
<p>你会发现，这和第 12 课「用 OpenAPI 规范解耦前后端」是<strong>同一种智慧</strong>：中间立一份机器/双方都认的契约，两端就都获得了不被对方绑架的自由。</p>
<div class="flow">
...
<p>为什么不干脆「一个供应商一套适配器」、省去「协议」这层抽象？因为那样会有大量<strong>重复</strong>。OpenRouter、xAI、本地的 Ollama，它们的 API 形态几乎和 OpenAI 一模一样——如果按供应商各写一套，就是把同一套「OpenAI 方言翻译」抄了三四遍，改一处就要改三四处。</p>
<p>opencode 把「方言」抽出来单独成「协议」，正是看准了「<strong>形态相同的供应商，本质共享同一种线缆格式</strong>」。于是供应商那一层变得极薄——它几乎只剩「我用哪种协议、怎么认证、连哪个端点」这三件事，真正费劲的「翻译」逻辑，全沉淀在为数不多的协议里复用。</p>
<p><strong>把重复的部分往下沉、把易变的部分往上提</strong>，是分层设计永恒的母题，这里又见一例。</p>
...
<p>这种「一切皆块」的设计，好处是<strong>结构清晰、可校验、可携带元数据</strong>——每个块都能单独挂一个 <span class="mono">cache_control</span>（下一节的主角）。这里还藏着一个有意思的不对称：普通的 <span class="mono">tool_use</span>（客户端工具）是「助手开口要求、由 opencode 在本地跑、再把 <span class="mono">tool_result</span> 回传」的<strong>三步往返</strong>；可那几个<strong>服务端工具</strong>（<span class="mono">web_search</span> / <span class="mono">code_execution</span> / <span class="mono">web_fetch</span>）却是供应商<strong>自己执行、把结果整块内联</strong>进助手这一轮——源码注释特意点明，它们「<strong>整块到达，没有流式增量</strong>」，没有客户端这边对应的 <span class="mono">tool_result</span> 往返。</p>
<p>一个「块」的类型标签，背后竟编码着「这工具该谁来跑」这样的执行语义差异，可见这套块类型设计之精细。还有个容易忽略的细节：<strong>系统提示（system）在 Anthropic 里不是一条消息，而是请求体顶层一个独立字段</strong>，类型是「一个文本块的数组」。</p>
<p>这跟 OpenAI 把 system 当成 messages 里第一条「role: system」的消息，是<strong>截然不同的方言</strong>——而这，正是为什么要有协议适配器：core 那边只有一份规范的 system，到了 Anthropic 这份 <span class="mono">body.from</span> 里，它被翻译成顶层 <span class="mono">system</span> 字段；到了第 31 课 OpenAI 那份里，它又被翻译成 messages 数组的头一条。<strong>同一个规范概念，两种方言两种摆法</strong>，这就是适配器在默默吸收的差异。</p>
<div class="cols">
...
<p>这就是「协议 ≠ 供应商」这条设计原则的<strong>现金回报</strong>。如果当初把协议和供应商揉成一团（每家一套 if 分支），新增一家「OpenAI 兼容」厂商就得复制粘贴一大坨编解码逻辑；而解耦之后，新增一家的成本，<strong>趋近于零</strong>。一个 24 行的文件，胜过千言万语的架构布道。</p>
<p>值得再多想一层的是：这 24 行之所以能这么短，<strong>不是因为偷懒，而是因为前面的功夫下到了位</strong>——正因为 Chat 协议把「请求编码」「流解码」干干净净地封装成了一个可复用的 <span class="mono">protocol</span> 对象，复用它才像「拿来即用」这么轻松。</p>
<p><strong>好的抽象，回报往往迟到，但一定会到</strong>：你在 Chat 协议上多花的那份「把编解码和网络分开」的心思，到这里、到每一个新接入的兼容厂商身上，连本带利地还给你。这也是为什么第 29 课要反复强调那张「两栏表」的边界——边界划得清，复用才来得爽。</p>
...
<p>This is the <strong>cash reward</strong> of the "protocol ≠ provider" design principle. Had protocol and provider been mashed together originally (a set of if-branches per vendor), adding one "OpenAI-compatible" vendor would mean copy-pasting a big lump of codec logic; after decoupling, the cost of adding one <strong>approaches zero</strong>. A 24-line file outweighs a thousand words of architecture preaching.</p>
<p>Worth one more layer of thought: these 24 lines can be this short <strong>not from laziness but because the earlier work was done right</strong>—precisely because the Chat protocol cleanly encapsulates "request encode" and "stream decode" into one reusable <span class="mono">protocol</span> object, reusing it is as easy as "plug and play."</p>
<p><strong>A good abstraction's reward often arrives late, but it arrives</strong>: the extra care you spent on the Chat protocol to "separate codec from network" is paid back here, with interest, at every newly-connected compatible vendor. This is why lesson 29 kept stressing that "two-column form"'s boundary—draw the boundary cleanly and reuse comes joyfully.</p>
...
<p>它的状态是一个 <span class="mono">FrameBufferState{buffer, offset}</span>：<span class="mono">buffer</span> 攒着收到的字节，<span class="mono">offset</span> 是已读到的位置。每来一个网络 chunk，就 <span class="mono">appendChunk</span> 把它接到 buffer 尾上（顺手把 offset 之前已消费的前缀<strong>压缩</strong>丢掉，于是 buffer 不会随流无限膨胀——最多比「还活着的窗口」多出一个 chunk）；然后 <span class="mono">consumeFrames</span> 一帧一帧地切：只要剩余字节够拼出一整帧，就用<strong>零拷贝的 subarray</strong> 切出来、验 CRC、吐给上层。</p>
<p>这台「字节→帧」的状态机，和第 29 课那台「帧→事件」的状态机，结构如出一辙——都是 <span class="mono">(状态, 新输入) → [新状态, 吐出的东西]</span>。<strong>流式解码原来是套娃的</strong>：framing 把字节攒成帧，protocol.stream 把帧攒成事件，两层各管一段、各是一台状态机，严丝合缝地叠在一起。第 29 课你以为只有一台状态机，到 Bedrock 这儿，你看见了它底下还垫着另一台。</p>
<p>这种「同一个模式在不同抽象层反复出现」的现象，在好的代码库里很常见——一旦你认得了「攒输入、够了就吐、留住未尽的部分继续攒」这个骨架，无论它出现在字节层、帧层还是事件层，你都能<strong>一眼认出老朋友</strong>，不必从头理解。这就是吃透一个核心模式的复利：它会在你读代码的余生里，一次次替你省下理解成本。</p>
...
<p>Its state is a <span class="mono">FrameBufferState{buffer, offset}</span>: <span class="mono">buffer</span> accumulates received bytes, <span class="mono">offset</span> is the read position. Each network chunk arriving, <span class="mono">appendChunk</span> joins it to the buffer's tail (incidentally <strong>compacting away</strong> the already-consumed prefix before offset, so the buffer doesn't grow unboundedly with the stream—at most one chunk past the "live window"); then <span class="mono">consumeFrames</span> cuts frame by frame: as long as remaining bytes suffice for a whole frame, cut it out with <strong>zero-copy subarray</strong>, verify CRC, emit upward.</p>
<p>This "byte→frame" state machine and lesson 29's "frame→event" state machine are structurally identical—both <span class="mono">(state, new input) → [new state, things emitted]</span>. <strong>Streaming decode is a nesting doll after all</strong>: framing accumulates bytes into frames, protocol.stream accumulates frames into events, two layers each owning a stretch, each a state machine, stacked seamlessly. Lesson 29 you thought there was only one state machine; here at Bedrock, you see another cushioning it underneath.</p>
<p>This "same pattern recurring at different abstraction layers" is common in good codebases—once you recognize the skeleton of "accumulate input, emit when enough, keep the unfinished part accumulating," wherever it appears—byte layer, frame layer, or event layer—you'll <strong>spot the old friend at a glance</strong>, no need to understand from scratch. That's the compound interest of mastering one core pattern: it saves you comprehension cost, again and again, for the rest of your reading life.</p>
...
<span class="cm">// 而一个工具回合里要复用很多次 → 稳赚。所以默认就开。</span></pre>
  <p>缓存写入比不缓存还贵一点点（1.25x），但读取便宜到几乎免费（0.1x）。<strong>只要前缀在 5 分钟内被复用哪怕一次，这笔买卖就划算</strong>；而一个工具回合动辄十几次往返，复用次数远不止一次——所以 opencode 把缓存<strong>设成默认开启</strong>（源码 <span class="mono">cache-policy.ts</span> 里只给了这笔成本账，没提任何外部框架）。还有个工程细节值得一提：<span class="mono">markMessageAt</span> 故意不用 <span class="mono">.map()</span> 而用 <span class="mono">slice + 下标赋值</span>，注释说得很实在——长对话每次请求都要跑它，<span class="mono">.map()</span> 的闭包派发和整数组复制「在性能剖析里会冒头」。<strong>连这种微小的热路径开销都被盯着</strong>，是生产级代码的底色，也是这套缓存逻辑敢于「默认开启」的底气所在。</p>
</div>
...
<span class="cm">// And a tool turn reuses many times → sure win. So on by default.</span></pre>
  <p>A cache write costs slightly more than no cache (1.25x), but a read is nearly free (0.1x). <strong>As long as the prefix is reused even once within 5 minutes, the deal pays off</strong>; and a tool turn easily has a dozen-plus round-trips, reusing far more than once—so opencode sets caching <strong>on by default</strong> (the source <span class="mono">cache-policy.ts</span> gives only this cost math, citing no external framework). One more engineering detail worth noting: <span class="mono">markMessageAt</span> deliberately avoids <span class="mono">.map()</span> and uses <span class="mono">slice + index assignment</span>, the comment frank about it—long conversations run it on every request, and <span class="mono">.map()</span>'s closure dispatch and whole-array copies "show up in profiling." <strong>Even such tiny hot-path overhead is watched</strong>—the hallmark of production code, and the confidence behind this cache logic daring to be "on by default."</p>
</div>
...
<div class="card analogy">
  <div class="tag">📖 生活类比</div>
  想象 opencode 是一家「万国翻译公司」，前六课我们招齐了翻译（协议）、配好了物流（传输）。可客户上门只会说一句：「我要找『克劳德·十四行诗』这位专家。」公司怎么从这个名字，立刻调出此人的<strong>完整履历卡</strong>：他会哪些活（能力）、收费多少（成本）、一次最多接多长的稿子（上下文上限）、走哪条物流线（Route）？靠的是一本<strong>随时更新的行业黄页</strong>（models.dev）——公司自己不硬记每个专家的资料，而是定期从这本公开黄页同步，新专家一上线就自动能查到。而 <strong>Copilot</strong> 这位「专家」尤其特别：他干的活和 OpenAI 那位<strong>一模一样</strong>（同一套协议），但进他家门得用<strong>一张特殊的门禁</strong>——先拿你的 GitHub 工牌去换一张当天有效的临时通行证，再凭通行证进门。<strong>同样的翻译本事，套一层独特的门禁系统</strong>，这就是 Copilot 的与众不同。
</div>
...
<div class="cellgroup">
  ...
  <div class="cell"><div class="c-tag">limit</div><div class="c-txt">context 上下文上限、input/output token 上限</div></div>
  <div class="cell"><div class="c-tag">modalities</div><div class="c-txt">输入/输出支持的模态：text/audio/image/video/pdf</div></div>
  <div class="cell"><div class="c-tag">family / release_date / status</div><div class="c-txt">模型家族、发布日期、alpha/beta/deprecated 状态</div></div>
</div>
<p>看到那个 <span class="mono">cost</span> 里的「缓存读 / 缓存写」单价了吗？这又是一根接通的线——第 34 课讲缓存默认开时，那句「写 1.25x、读 0.1x、复用一次即回本」的精算，算盘珠子<strong>就拨在这些来自 models.dev 的单价上</strong>。模型大全不只是一本「通讯录」，它还是缓存决策、用量计费的<strong>事实地基</strong>。<span class="mono">models-dev.ts</span> 通过 HTTP 把这份目录拉下来、缓存住，并在刷新时广播一个 <span class="mono">models-dev.refreshed</span> 事件：</p>
<div class="flow">
  <div class="f-node">models.dev<br><small>社区公开目录</small></div>
  <div class="f-arrow">HTTP 拉取 →</div>
  <div class="f-node">本地缓存<br><small>解析成 Model/Provider</small></div>
  <div class="f-arrow">刷新时 →</div>
  <div class="f-node">广播 refreshed<br><small>models-dev.refreshed 事件</small></div>
</div>
<p><strong>把易变的事实外置到一个会自我更新的目录</strong>，这样新模型一上线，opencode 往往一行代码都不用改就能认得它——这和第 31 课「OpenAI 兼容白嫖生态」是同一种「把变化挡在代码之外」的智慧，只不过那次挡的是「协议的变化」，这次挡的是「模型事实的变化」。</p>
<p>再往深想一层，这其实是一种<strong>责任的转移</strong>：模型的价格、上限、能力，本该由「最了解它们、且最频繁更新它们」的人来维护——而那不是 opencode 的开发者，是 models.dev 这个社区。opencode 只负责「消费」这份目录，把「维护事实」的脏活累活，交给了一个专门做这件事的上游。<strong>不重复造轮子，也不重复维护事实</strong>，这是更高一层的复用。</p>
...
<p>注意这种设计的体贴：它<strong>不是返回一个含糊的 null 让上层去猜</strong>「到底哪一步出了错」，而是明确告诉你「是供应商没找到，还是供应商有、但这个模型没找到」。两种失败的修复方式截然不同——前者你得检查 provider 名、后者你得检查 model 名——把它们用不同的错误类型分开，上层就能给出精准的提示。这种把失败<strong>分得清清楚楚</strong>的设计，呼应了整本书反复出现的「让错误自己说清自己是谁」。</p>
<p>这一步解析的产物 <span class="mono">ModelInfo</span> 只装<strong>模型事实</strong>（能力、成本、上限、形态），并<strong>不</strong>包含 Route 本身；真正的 Route 是<strong>另一步</strong>由各家的 provider 门面（<span class="mono">packages/llm/src/providers/*.ts</span>）按这些事实加配置拼出来的——也就是第 33 课那几个正交件的组合。<strong>至此，一个用户输入的模型名，终于接上了一条从协议到传输、配置完整的链路。</strong></p>
...
<p>它和第 31 课的「openai-compatible 只改端点」是同一招的升级版——这次不光改端点，还<strong>换了一套认证</strong>，但<strong>协议依然原封不动地复用</strong>。Copilot 的认证比一般的 API key 复杂：你不能直接拿一个固定密钥，而要先用 GitHub 的 OAuth 工牌，去<strong>换取一个有时效的 Copilot 专用 token</strong>，再用它做 Bearer 认证。</p>
<p>正因为 <span class="mono">auth</span> 早被设计成独立的一件，这套「先换证、再进门」的特殊流程，才能<strong>整个塞进 auth 这一件里、丝毫不惊动协议层</strong>。假如当初认证逻辑和协议编解码缠在一起，Copilot 这种「同协议、异认证」的家伙就会逼你把 OpenAI 协议又抄一遍——而正交分解让它优雅地化解于无形。<strong>用一个最棘手的真实供应商，给整个 M6 的设计哲学盖了章。</strong></p>
...
<div class="card macro">
  <div class="tag">🗺️ 宏观图景 · M6 全景收官</div>
  <p>这一课补完模型解析，整个 M6（LLM 协议层）的闭环正式合龙。回望这一整个 part：</p>
  <ul>
...
<div class="card analogy">
  <div class="tag">📖 Analogy</div>
  Imagine opencode is a "universal translation company." Over six lessons we hired the interpreters (protocols) and arranged logistics (transport). But a client walks in saying just: "I want the expert 'Claude Sonnet.'" How does the company instantly pull up this person's <strong>complete résumé card</strong> from that name: what skills they have (capabilities), their fees (cost), the longest manuscript they take at once (context limit), which logistics line they use (Route)? Via a <strong>constantly-updated industry yellow pages</strong> (models.dev)—the company doesn't memorize each expert's data, but periodically syncs from this public directory, so a new expert is auto-queryable the moment they come online. And <strong>Copilot</strong>, this "expert," is especially special: the work he does is <strong>identical</strong> to OpenAI's (same protocol), but entering his door requires <strong>a special access pass</strong>—first take your GitHub badge to exchange for a temporary same-day pass, then enter with that pass. <strong>The same translation skill, wrapped in a distinctive access system</strong>—that's what sets Copilot apart.
</div>
...
<div class="cellgroup">
  ...
  <div class="cell"><div class="c-tag">limit</div><div class="c-txt">context cap, input/output token caps</div></div>
  <div class="cell"><div class="c-tag">modalities</div><div class="c-txt">input/output supported modalities: text/audio/image/video/pdf</div></div>
  <div class="cell"><div class="c-tag">family / release_date / status</div><div class="c-txt">model family, release date, alpha/beta/deprecated status</div></div>
</div>
<p>See those "cache read / cache write" prices in <span class="mono">cost</span>? Another connected thread—lesson 34's "write 1.25x, read 0.1x, one reuse pays off" reckoning was computed <strong>against exactly these prices from models.dev</strong>. The model encyclopedia isn't just an "address book," it's the <strong>factual bedrock</strong> for caching decisions and usage billing. <span class="mono">models-dev.ts</span> fetches this catalog over HTTP, caches it, and broadcasts a <span class="mono">models-dev.refreshed</span> event on refresh:</p>
<div class="flow">
  <div class="f-node">models.dev<br><small>community public catalog</small></div>
  <div class="f-arrow">HTTP fetch →</div>
  <div class="f-node">local cache<br><small>parsed into Model/Provider</small></div>
  <div class="f-arrow">on refresh →</div>
  <div class="f-node">broadcast refreshed<br><small>models-dev.refreshed event</small></div>
</div>
...
<table class="t">
  ...
  <tr><td>endpoint</td><td>api.openai.com</td><td>Copilot's own baseURL</td></tr>
...
<div class="card macro">
  <div class="tag">🗺️ The Big Picture · M6 Panoramic Finale</div>
  <p>This lesson completes model resolution, and the whole M6 (LLM protocol layer) loop formally closes. Looking back over this entire part:</p>
  <ul>
