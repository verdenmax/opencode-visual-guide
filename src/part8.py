...
<p>那「作用域（scoped）」又是什么意思？多台 MCP 服务器可能各有一个叫 <span class="mono">search</span> 的工具，怎么不打架？opencode 的办法是给每个工具名<strong>冠上服务器名做前缀</strong>。名字先经 <span class="mono">sanitize</span> 洗一遍（把非 <span class="mono">[a-zA-Z0-9_-]</span> 的字符替换成 <span class="mono">_</span>），再用 <span class="mono">_</span> 拼起来：</p>
<div class="cellgroup">
  <div class="cell"><div class="c-tag">sanitize</div><div class="c-txt"><span class="mono">/[^a-zA-Z0-9_-]/g → "_"</span>：洗掉工具名里不合法的字符</div></div>
  <div class="cell"><div class="c-tag">命名规则</div><div class="c-txt"><span class="mono">sanitize(服务器名) + "_" + sanitize(工具名)</span></div></div>
  <div class="cell"><div class="c-tag">例子</div><div class="c-txt">github 服务器的 search → <span class="mono">github_search</span>；jira 的 search → <span class="mono">jira_search</span>，互不冲突</div></div>
</div>
<p>这个带前缀的名字，就是 MCP 工具的「作用域」——它既避免了重名冲突，又让模型一眼看出某工具来自哪台服务器。而这串名字最妙的去处在 <span class="mono">session/tools.ts</span>：MCP 工具被并入会话工具集时，那道权限闸门写的是 <span class="mono">ctx.ask({ permission: key, ... })</span>——<strong>这个 key，正是上面那个带服务器前缀的作用域名</strong>。也就是说，MCP 工具<strong>和内置工具走的是同一套权限系统</strong>（第 41 课的 Ruleset），你完全可以在配置里写「<span class="mono">github_*</span> 一律放行、<span class="mono">jira_delete_*</span> 必须问」这样的规则，精确管控每台外部服务器、每个外部工具的权限。这就是开头那句「待遇是一等公民」的落地：<strong>外来工具一旦进门，就和原生工具共享注册表、权限、插件钩子，没有半点二等待遇。</strong></p>
...
<div class="card macro">
  ...
  <ul>
    ...
    <li><strong>作用域</strong>：工具名 = <span class="mono">sanitize(服务器名) + "_" + sanitize(工具名)</span>，避免多服务器重名冲突。这个作用域名同时是<strong>权限 key</strong>——MCP 工具在 <span class="mono">session/tools.ts</span> 走<strong>和内置工具同一道权限闸门</strong>（L41 Ruleset）、同一批插件钩子。出身二等、待遇一等。</li>
...
<span class="kw">for</span> (<span class="kw">const</span> [key, item] <span class="kw">of</span> Object.<span class="fn">entries</span>(<span class="kw">yield*</span> mcp.<span class="fn">tools</span>())) {
  ...
  item.execute = (args, opts) =&gt;
    run.<span class="fn">promise</span>(Effect.<span class="fn">gen</span>(<span class="kw">function*</span> () {
      <span class="cm">// ← 和内置工具同一道权限闸门，key=「服务器名_工具名」作用域名</span>
  ...
  <p>注意那行 <span class="mono">ctx.ask({ permission: key, ... })</span>——<span class="mono">key</span> 就是 <span class="mono">github_search</span> 这样的作用域名。这意味着 MCP 工具<strong>没有自己的一套权限逻辑</strong>，而是<strong>复用</strong>了第 41 课那套统一的权限系统：同样的 ask（allow/ask/deny 三态）、同样的 Ruleset 匹配。这正是「待遇一等公民」最硬的证据——<strong>系统没有为「外来工具」开任何后门，它必须和原生工具走完全相同的安全检查</strong>。这种「不为扩展点破例」的纪律极其重要：一旦你为某类工具开了「免检」绿色通道，整个权限系统的可信度就崩了。opencode 的做法是反过来的——<strong>越是外来的、越不可控的工具（谁知道某个第三方 MCP 服务器会让你的 agent 干什么），就越要让它老老实实排进同一道闸门。</strong></p>
</div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>作用域 = 权限 key</strong>：工具名 = <span class="mono">sanitize(服务器名)+"_"+sanitize(工具名)</span>（如 <span class="mono">github_search</span>），既防重名，又作为 <span class="mono">session/tools.ts</span> 里的权限 key——MCP 工具走<strong>和内置工具同一道权限闸门</strong>（L41）。出身二等，待遇一等。</li>
...
<div class="cellgroup">
  ...
  <div class="cell"><div class="c-tag">naming rule</div><div class="c-txt"><span class="mono">sanitize(serverName) + "_" + sanitize(toolName)</span></div></div>
  <div class="cell"><div class="c-tag">example</div><div class="c-txt">github server's search → <span class="mono">github_search</span>; jira's search → <span class="mono">jira_search</span>, no collision</div></div>
</div>
<p>This prefixed name is the MCP tool's "scope"—it avoids name collisions and lets the model see at a glance which server a tool comes from. And this name's finest destination is in <span class="mono">session/tools.ts</span>: when MCP tools merge into the session's tool set, the permission gate writes <span class="mono">ctx.ask({ permission: key, ... })</span>—<strong>this key is exactly that server-prefixed scoped name</strong>. That is, MCP tools <strong>go through the same permission system as built-in tools</strong> (L41's Ruleset); you can perfectly well write rules in config like "<span class="mono">github_*</span> all allowed, <span class="mono">jira_delete_*</span> must ask," precisely governing each external server's, each external tool's permission. That's the landing of the opening's "first-class treatment": <strong>once a foreign tool is in the door, it shares registry, permissions, plugin hooks with native tools—no second-class treatment whatsoever.</strong></p>
...
<p>这里有个容易被忽略却很关键的细节：那句 <span class="mono">import("@ai-sdk/alibaba")</span> 是<strong>动态导入</strong>。意味着只有当你<strong>真的用到</strong>阿里的模型时，阿里的 SDK 包才会被加载进内存——你没用到的三十多家 SDK，一行代码都不会跑、一点内存都不占。三十多个 provider 插件虽然全都「在场待命」，但每个都<strong>极其廉价</strong>（不到 20 行 + 一个延迟加载的 import），真正昂贵的 SDK 加载严格按需发生。<p>这又是那个全书主题——<strong>「便宜地在场，昂贵地按需」</strong>——在 provider 层的又一次回响。那么这三十多家「在场待命」的供应商都有谁？<span class="mono">ProviderPlugins</span> 数组里大致可分几类：</p>
<table class="t">
