...
<p>看这张表你就明白：<strong>配置不是某个孤立模块的设置，而是把全系统的「用户可调项」汇到一处的总枢纽</strong>。第 41 课的权限、第 42 课的输出上限、第 43 课的 skills、第 46 课的 MCP……它们各自的「该怎么配」，最终都落到 <span class="mono">Config.Info</span> 的某一格里。而它是一个<strong>类型化的 Schema 类</strong>——这意味着配置文件在加载时就被<strong>校验</strong>：写错了字段、给错了类型，在解析关就被挡下（又是第 5、22 课「schema 即契约」的体现），而不是等到某个子系统运行时才神秘出错。</p>
<p><strong>一份配置文件，是用户向整个系统下达意图的统一入口；一个 Config.Info schema，是这份意图的类型化契约。</strong>这种「<strong>把全系统的可调项收拢到一个类型化结构里</strong>」的做法本身就是一种设计美德：用户不必去十几个地方改十几种格式的设置，只在一处声明；而每个子系统也不必各自发明一套读配置的办法，都从这同一份 <span class="mono">Config.Info</span> 里取自己那一格。<strong>一处声明、各取所需</strong>，配置因此既好写、又好校验、又好演进——加一个新可调项，就是给 schema 添一个字段。</p>
...
<p>这套「<strong>保留一串有序来源、按 key 逐项就近裁决</strong>」的设计，比「急着把所有配置深合并成一个大对象」要干净得多。它不预先算一份合成结果，而是<strong>把来源原样按优先级留着，谁问哪个 key，就当场为那个 key 裁决一次</strong>。好处是：每一项的「<strong>它从哪份配置来的、为什么是这个值</strong>」始终清清楚楚、可追溯——你要排查「这个 model 怎么是这个」，顺着优先级一层层看就行，不会迷失在一个早已合并、来源难辨的大对象里。</p>
<p><strong>把『有哪些来源』和『某一项最终取谁』分开，是这套配置系统可解释性的根基。</strong>这种可解释性在现实里极其值钱：当一个团队成员困惑「为什么我的 agent 用了这个奇怪的模型」，答案不是去翻一个早已揉成一团的合成对象，而是顺着那串有序 Entry 一层层问「这一层定义 model 了吗」，立刻就能定位到「哦，是我自己子目录里那份 .opencode 写的」。</p>
<p><strong>配置出问题时最怕的就是『来源成谜』，而这套设计从结构上就让来源始终可追。</strong>这和第 41 课权限的 <span class="mono">findLast</span>「后匹配者胜」、第 37 课注册表的「摞顶最新」，又是同一种「有序来源、就近取值」的思路在不同地方的回响。</p>
...
<p>From this table you see: <strong>config isn't some isolated module's settings but the hub gathering the whole system's "user-tunable items" in one place</strong>. Lesson 41's permissions, lesson 42's output cap, lesson 43's skills, lesson 46's MCP… their respective "how to configure" all land in a cell of <span class="mono">Config.Info</span>.</p>
<p>And it's a <strong>typed Schema class</strong>—meaning the config file is <strong>validated</strong> at load: a wrong field, a wrong type, gets stopped at the parse gate (again lessons 5, 22's "schema is contract"), not mysteriously erroring at some subsystem's runtime later. <strong>A config file is the user's unified entry for issuing intent to the whole system; a Config.Info schema is the typed contract of that intent.</strong></p>
<p>This "<strong>gathering the whole system's tunables into one typed structure</strong>" is itself a design virtue: the user needn't change a dozen settings in a dozen formats in a dozen places, but declares in one; and each subsystem needn't invent its own way to read config, all taking its cell from this same <span class="mono">Config.Info</span>. <strong>Declare in one place, each takes what it needs</strong>—config is thus easy to write, validate, and evolve—adding a new tunable is adding a field to the schema.</p>
...
<div class="card detail">
  <div class="tag">🔬 源码细节</div>
  <p>plan agent 的权限定义，把「只读规划」写成了几条声明（简化自 opencode 的 agent 定义；注意这段用的是<strong>运行期</strong> agent 的字段名 <span class="mono">permission</span>/<span class="mono">native</span>，与前面 <span class="mono">AgentV2.Info</span> schema 里的 <span class="mono">permissions</span>/<span class="mono">system</span> 是<strong>同一概念的两种写法</strong>）：</p>
...
<div class="card detail">
  <div class="tag">🔬 Source Detail</div>
  <p>The plan agent's permission definition writes "read-only planning" as a few declarations (simplified from opencode's agent definition; note this snippet uses the <strong>runtime</strong> agent's field names <span class="mono">permission</span>/<span class="mono">native</span>, the <strong>same concepts in a different representation</strong> from <span class="mono">AgentV2.Info</span>'s <span class="mono">permissions</span>/<span class="mono">system</span>):</p>
