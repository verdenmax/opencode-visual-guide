...
<p>这里有个非常务实的设计：V1 数据搬进 SQLite 时，<strong>并没有立刻被「翻译」成 V2 结构</strong>——它仍以 V1-shaped 的 JSON 原样躺在 <span class="mono">message</span>/<span class="mono">part</span> 表的 <span class="mono">data</span> 列里（这正是 L49 看到的「V1 同堂」之源）。真正的「V1→V2 语义转换」是<strong>读取时按需发生</strong>的：<span class="mono">message-v2.ts</span> 从 <span class="mono">PartTable</span>/<span class="mono">MessageTable</span> 读出 V1 行，<strong>当场投影（project）成 V2 的消息结构</strong>交给上层。这种「<strong>先把字节原样搬过来、语义转换推迟到读取时</strong>」的策略极其聪明：它让<strong>数据迁移这一步变得简单又安全</strong>——只是把文件内容照搬进表，不动一个字节的语义，出错风险最小；而复杂易变的「V1 语义怎么映射成 V2」逻辑则集中在读取路径上一处维护、随时可改。<strong>把「搬运」和「翻译」解耦，让最危险的批量数据迁移退化成最无脑的字节复制</strong>——这是大型数据迁移里一条值得反复回味的智慧。它还顺带带来一个好处：万一某天发现「V1→V2 的语义映射」写错了，你<strong>不必重跑那场代价高昂的数据大迁移</strong>，只要改读取时的投影逻辑、重新读一遍即可，因为原始 V1 字节<strong>始终原封不动地保存在库里</strong>。原始数据不被破坏性改写，就永远留着一条「重新解释」的退路——这种「保真原始、按需解释」的克制，和 L42 有界输出「全文 spill、按需回取」、L43 skills「名字常驻、正文按需」是同一种深谋远虑。</p>
...
<p>这一课有两个最值得带走的设计。其一，<strong>压缩 = 「留近况原文 + 把远的折成结构化摘要」</strong>：当对话的 token 估算超出上下文窗口（减去输出余量），opencode 自动把最近约 8000 token 的对话<strong>原样保留</strong>、把更早的对话交给 LLM<strong>填进一张固定结构的摘要模板</strong>（目标/约束/进展/决策/下一步/关键上下文/相关文件），而且这份摘要是<strong>「锚定式」增量更新</strong>的——下次压缩不重写，而是在旧摘要上「保留仍成立的、删过时的、并入新的」。其二，<strong>快照 = 「影子 git 仓库」</strong>：opencode 不动你真实的 <span class="mono">.git</span>，而是在数据目录下另起一个独立的 git 仓库（<span class="mono">--git-dir</span> 指向它、<span class="mono">--work-tree</span> 指向你的真实工作目录），用 <span class="mono">git write-tree</span> 给工作目录拍一张「树快照」、用 <span class="mono">read-tree</span>+<span class="mono">checkout-index</span> 还原。<strong>它把 git 强大的内容寻址存储借来当时间机器，却绝不污染你自己的提交历史。</strong>读懂这两笔，你就懂了「一个 agent 如何在有限的上下文里装下无限长的对话，又如何安全地给你一颗『后悔药』」。</p>
...
<p>最见功力的是<strong>「锚定式增量摘要」</strong>：如果这场对话<strong>之前已经压缩过</strong>（历史里已有一条 <span class="mono">compaction</span> 消息），这次不会从头重写摘要，而是把旧摘要塞进提示里，让模型「<strong>用上面的对话历史更新这份锚定摘要：保留仍成立的细节、删掉过时的、并入新事实</strong>」。为什么这很重要？因为摘要本身会被<strong>反复压缩</strong>——如果每次都从仅剩的近况重新生成，越早的信息会在一次次压缩中被<strong>逐渐稀释、最终遗忘</strong>。而「锚定 + 增量更新」让那份摘要像一条<strong>不断被修订、却始终连续的主线</strong>，把整场对话从头到尾的关键脉络<strong>一路传下去</strong>，不因压缩而断裂。这和 L42「有界输出」是同一种哲学在不同层级的回响：L42 管<strong>单个工具输出</strong>别撑爆上下文，压缩管<strong>整场对话</strong>别撑爆上下文——稀缺的永远是模型那点宝贵的注意力。</p>
