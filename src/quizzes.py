    ...
    "51-compaction-snapshots.html": {
        "mcq": [
            ...
            },
            {
                ...
                "answer": 0,
                "why": {"zh": "revert.ts 让你退回某条消息（messageID，可细到 partID），做两件事的合体：退对话（裁掉该消息之后的消息）+退文件（snap.restore 把工作目录还原到那时的快照）。为什么必须一起退？因为单退对话不退文件，会面对「对话以为没改过、文件却已被改」的错乱；单退文件不退对话同理——唯有两者作为整体一起回到过去，那一刻的世界才自洽。可反悔的关键：revert 在还原前先 snap.track() 给「现在」拍快照、存进 L49 session 表那个 revert:{messageID,partID?,snapshot?,diff?} JSON 列，于是 unrevert 能 restore 回 revert 之前——后悔药也有后悔药。状态被持久化，重启仍有效。这正是 L49 session.revert 列的用武之地、L51 compaction(对话记忆)+snapshot(文件记忆)的合体。", "en": "revert.ts lets you roll back to a message (messageID, down to partID), a fusion of two things: rewind conversation (trim messages after it) + rewind files (snap.restore restores the working dir to that snapshot). Why must they roll back together? Because rewinding only the conversation not files faces the disorder of \"the conversation thinks nothing changed yet files are changed\"; rewinding only files not conversation likewise—only both, as a whole, returning to the past is that moment's world self-consistent. The key to reversibility: before restoring, revert first snap.track()s a snapshot of \"now,\" storing it in L49 session-table's revert:{messageID,partID?,snapshot?,diff?} JSON column, so unrevert can restore back to before revert—even the regret pill has a regret pill. The state is persisted, works after restart. This is exactly where L49's session.revert column earns its keep, and L51's fusion of compaction (conversation memory) + snapshot (file memory)."},
            },
        ],
        "open": [
