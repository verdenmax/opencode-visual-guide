...
QUIZZES = {
    "69-ecosystem-tour.html": {
        "mcq": [
            {
                "q": {"zh": "课里把 opencode 的整个生态收束为一句话。最准确的是哪个？", "en": "The lesson distills opencode's whole ecosystem into one line. Which is most accurate?"},
                "opts": [
                    {"zh": "<strong>一台引擎、多副面孔与接口</strong>：内核 server 只有一个（L09），TUI/web/桌面/Slack/可分享云视图都只是<strong>连上它的客户端</strong>，共享同一套 SDK（L12）+ 事件流（L11）——正因内核「数据与表现分离、广开门户」，才能长出这么多面孔而不重复造轮子", "en": "<strong>One engine, many faces and interfaces</strong>: the core server is one (L09), and TUI/web/desktop/Slack/shareable cloud view are merely <strong>clients connecting to it</strong>, sharing the same SDK (L12) + event stream (L11)—because the core is \"data separated from presentation, doors wide open,\" it grows so many faces without reinventing wheels"},
                    {"zh": "每种前端都是一个独立的 opencode 实现，各有各的内核", "en": "Each frontend is an independent opencode implementation with its own core"},
                    {"zh": "opencode 其实是好几个互不相关的开源项目的合集", "en": "opencode is actually a collection of several unrelated open-source projects"},
                    {"zh": "桌面版、网页版、TUI 用的是三套完全不同的会话逻辑", "en": "The desktop, web, and TUI use three completely different session logics"},
                ],
                "answer": 0,
                "why": {"zh": "全书最根本的架构决定（L03/L09）：内核跑成<strong>一个 server</strong>，所有界面都只是连它的客户端。这一课是这条线（连同 L56「数据与表现分离」、L61「ACP 让编辑器驱动」）的最终展开：web app（<span class=\"mono\">packages/app</span>）、Electron 桌面（<span class=\"mono\">desktop</span>，套壳 web app）、TUI、Slack bot、可分享云视图……全经<strong>同一套 SDK + 事件流连同一个 server</strong>，共享同一份会话/工具/上下文逻辑。桌面版甚至只是「给 web app 套个 Electron 壳」，连界面都不重写。所以不是「各有各的内核」「互不相关项目合集」「三套不同会话逻辑」——恰恰相反，<strong>同一个内核，多副皮</strong>。", "en": "The book's most fundamental architectural decision (L03/L09): the core runs as <strong>one server</strong>, all interfaces merely clients connecting to it. This lesson is that thread's final unfolding (with L56 \"data separated from presentation,\" L61 \"ACP lets editors drive\"): the web app (<span class=\"mono\">packages/app</span>), Electron desktop (<span class=\"mono\">desktop</span>, shelling the web app), TUI, Slack bot, shareable cloud view… all connect to one server via <strong>the same SDK + event stream</strong>, sharing one session/tool/context logic. The desktop is even just \"put an Electron shell on the web app,\" not rewriting the UI. So it's not \"each its own core,\" \"unrelated projects,\" or \"three different session logics\"—on the contrary, <strong>one core, many skins</strong>."},
            },
            {
                "q": {"zh": "会话分享（<span class=\"mono\">share(sessionID)→{url}</span>）让别人点开链接就能在线只读看你的会话。它为什么能做得这么轻巧？", "en": "Session sharing (<span class=\"mono\">share(sessionID)→{url}</span>) lets others open a link to watch your session read-only online. Why can it be so lightweight?"},
                "opts": [
                    {"zh": "因为它<strong>建在 L65 的单写入事件溯源同步之上</strong>：会话本就是一条带全序号、可重放的事件流，「分享」无非把这条流转发到云端一个 Cloudflare <strong>Durable Object</strong>（<span class=\"mono\">SyncServer</span>）中转，旁观者经 WebSocket 重放追平——正是「单写入者写、他设备只读重放」在云端的落地", "en": "Because it's <strong>built atop L65's single-writer event-sourcing sync</strong>: a session is already a totally-numbered, replayable event stream, so \"sharing\" is just forwarding that stream to a cloud Cloudflare <strong>Durable Object</strong> (<span class=\"mono\">SyncServer</span>) relay, and watchers replay to catch up over WebSocket—exactly \"single writer writes, other devices read-only replay\" landed in the cloud"},
                    {"zh": "因为它把你的整个本地数据库实时上传到公网供人下载", "en": "Because it uploads your entire local database to the public internet for download"},
                    {"zh": "因为分享时会让旁观者也能写你的会话，多人共写", "en": "Because sharing lets watchers also write your session, multi-writer"},
                    {"zh": "因为它给每个旁观者都跑了一个完整的 opencode 内核", "en": "Because it runs a full opencode core for each watcher"},
                ],
                "answer": 0,
                "why": {"zh": "分享之所以轻巧，根在 L65 那块地基：会话已经是一条<strong>单写入、带单调 seq 全序、可重放</strong>的事件流。<span class=\"mono\">opencode/src/share</span> 的 <span class=\"mono\">share(sessionID)→{url}</span> 把这条流推到 <span class=\"mono\">packages/function</span> 的一台 Cloudflare <strong>Durable Object</strong>（<span class=\"mono\">SyncServer extends DurableObject</span>，WebSocket+存储，按 <span class=\"mono\">session/</span> 前缀存）中转，旁观者打开链接就经 WebSocket 订阅、重放事件追平——你的浏览器/手机就是 L65 说的「其他（只读）设备」。注意是<strong>只读</strong>（单写入约束没变），不是多人共写；也不需要给每个旁观者跑完整内核（他们只重放事件）。这正是「好的底层抽象会在意想不到的上层开花」（同 L61 move-session 建在 Location 解耦上）。", "en": "Sharing is lightweight thanks to L65's foundation: a session is already a <strong>single-writer, monotonic-seq totally-ordered, replayable</strong> event stream. <span class=\"mono\">opencode/src/share</span>'s <span class=\"mono\">share(sessionID)→{url}</span> pushes this stream to a Cloudflare <strong>Durable Object</strong> in <span class=\"mono\">packages/function</span> (<span class=\"mono\">SyncServer extends DurableObject</span>, WebSocket+storage, stored by <span class=\"mono\">session/</span> prefix) as a relay; watchers open the link, subscribe over WebSocket, and replay events to catch up—your browser/phone is L65's \"other (read-only) device.\" Note it's <strong>read-only</strong> (the single-writer constraint unchanged), not multi-writer; nor does it run a full core per watcher (they only replay events). This is \"a good low-level abstraction blossoms in unexpected upper layers\" (like L61 move-session built on Location decoupling)."},
            },
            {
                "q": {"zh": "课里说这一课「不教新机制，而教一种看待全局的眼光」。这种眼光具体是什么？", "en": "The lesson says it \"teaches no new mechanism but a way of seeing the whole.\" What specifically is that lens?"},
                "opts": [
                    {"zh": "面对一个长出一大圈周边的系统，<strong>别被表面的五花八门迷惑，去找那个大家共享的核心</strong>——找到内核，再看每一圈周边（前端/分享/集成/Slack/基础设施）如何围着它生长，整个生态的逻辑就一目了然", "en": "Facing a system that has grown a large surrounding ring, <strong>don't be dazzled by the surface variety—find the core everyone shares</strong>—find the core, then see how each ring (frontends/sharing/integration/Slack/infrastructure) grows around it, and the whole ecosystem's logic becomes clear"},
                    {"zh": "把每个周边包都逐行读完，才能理解系统", "en": "Read every peripheral package line by line to understand the system"},
                    {"zh": "优先看 star 最多的那个包，它最重要", "en": "Look at the most-starred package first; it's the most important"},
                    {"zh": "先看部署脚本，从运维倒推架构", "en": "Look at deployment scripts first, inferring architecture backward from ops"},
                ],
                "answer": 0,
                "why": {"zh": "这一课作为收官，给的是一种<strong>读源码/看系统的方法论</strong>：当一个系统长出一大圈周边（TUI/web/desktop/share/integration/slack/enterprise/infra），别去数它有多少零件、被表面的繁多迷惑，而要先找那个<strong>大家共享的核心</strong>——opencode 里就是那个「拥有一切」的 server（L09）。抓住核心，再看每一圈周边如何围着它生长、靠什么连它（同一套 SDK+事件流），整个生态的脉络就清楚了。从 <span class=\"mono\">package.json</span> 依赖图就能读出「大家都连一个内核」这件事。所以不是「逐行读完每个包」（鸟瞰不需要）、不是「看 star/部署脚本」——核心方法是<strong>先定位共享内核、再看周边如何围绕</strong>。", "en": "As the finale, this lesson gives a <strong>methodology for reading source/understanding systems</strong>: when a system grows a large ring (TUI/web/desktop/share/integration/slack/enterprise/infra), don't count its parts or get dazzled by the surface multitude—first find the <strong>core everyone shares</strong>—in opencode, the \"owns everything\" server (L09). Grasp the core, then see how each ring grows around it and what connects it (the same SDK+event stream), and the ecosystem's threads become clear. The <span class=\"mono\">package.json</span> dependency graph itself reveals \"everyone connects to one core.\" So it's not \"read every package line by line\" (a bird's-eye view needn't), nor \"look at stars/deploy scripts\"—the core method is <strong>locate the shared core first, then see how the periphery surrounds it</strong>."},
            },
        ],
        "open": [
            {"zh": "这是全书最后一课，它把 opencode 收束为「<strong>一台引擎、多副面孔与接口</strong>」，并指出整个生态（多前端、分享云同步、集成、Slack、企业、基础设施）之所以能不重复造轮子地共存，全靠它们围着同一个「数据与表现分离、广开门户」的内核 server（L09/L56/L61）。请你回顾整本书，谈谈这条「<strong>一个内核、多副面孔</strong>」的主线是怎样一步步铺垫出来的（L03 客户端/服务器 → L09 server 拥有一切 → L11/L12 SDK+事件流 → L52–56 TUI → L57–61 扩展/ACP → 本课生态），并谈谈这种架构在「可维护性、可扩展性、生态繁荣」上的根本优势，以及它对你设计自己的系统有什么启发。", "en": "This is the book's final lesson, distilling opencode as \"<strong>one engine, many faces and interfaces</strong>,\" noting the whole ecosystem (multiple frontends, share+cloud sync, integration, Slack, enterprise, infrastructure) coexists without reinventing wheels because they surround the same \"data separated from presentation, doors wide open\" core server (L09/L56/L61). Looking back over the whole book, discuss how this \"<strong>one core, many faces</strong>\" throughline was laid step by step (L03 client/server → L09 server owns everything → L11/L12 SDK+event stream → L52–56 TUI → L57–61 extensions/ACP → this lesson's ecosystem), the fundamental advantages of this architecture for \"maintainability, extensibility, ecosystem flourishing,\" and what it inspires for designing your own systems."},
            {"zh": "课里把这一课当作一种「读源码的眼光」：面对一个庞大系统，先找那个大家共享的核心，再看每一圈周边如何围着它生长——并指出「大家都连一个内核」这件事甚至能从 <span class=\"mono\">package.json</span> 的依赖图里直接读出来。请你谈谈你对这种「<strong>从依赖关系/数据流入手，快速摸清一个陌生大型代码库骨架</strong>」方法的理解：除了依赖图，你还会用哪些线索（入口文件、事件/消息流向、最被依赖的模块、目录结构）来定位一个系统的「核心」？再以这本书带你读 opencode 的整个过程为例，谈谈「<strong>先骨架后细节、自底向上</strong>」地读一个大项目，相比「从头到尾逐文件读」有什么优势。", "en": "The lesson treats this as a \"source-reading lens\": facing a large system, find the shared core first, then see how each ring grows around it—noting \"everyone connects to one core\" is even readable straight from the <span class=\"mono\">package.json</span> dependency graph. Discuss your understanding of this method of \"<strong>starting from dependencies/data flow to quickly grasp an unfamiliar large codebase's skeleton</strong>\": besides the dependency graph, what clues would you use (entry files, event/message flow, the most-depended-on module, directory structure) to locate a system's \"core\"? Then, using this book's whole journey through opencode as an example, discuss the advantages of reading a large project \"<strong>skeleton first, details later, bottom-up</strong>\" vs \"reading file by file front to back.\""},
        ],
    },
    ...
    "46-mcp.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "工具名 = sanitize(服务器名)+\":\"+sanitize(工具名)（如 github:search、jira:search），这个带前缀的作用域名同时就是 session/tools.ts 里的权限 key", "en": "tool name = sanitize(serverName)+\":\"+sanitize(toolName) (e.g. github:search, jira:search); this prefixed scoped name is also the permission key in session/tools.ts"},
                ...
                "answer": 0,
                "why": {"zh": "opencode 给每个工具名冠上服务器名前缀：先 sanitize（把非 [a-zA-Z0-9_-] 的字符替换成 _）洗一遍名字，再用 : 拼成 sanitize(服务器名)+\":\"+sanitize(工具名)。于是 github 的 search → github:search、jira 的 search → jira:search，互不冲突。这串带前缀的「作用域名」最妙的去处在 session/tools.ts：MCP 工具并入会话工具集时，权限闸门写 ctx.ask({ permission: key })——这个 key 正是作用域名。所以你能在配置里写「github:* 一律放行、jira:delete_* 必须问」，精确管控每台外部服务器、每个外部工具。命名既解决重名，又直接成为权限系统的抓手。", "en": "opencode prefixes each tool name with the server name: first sanitize (replace non-[a-zA-Z0-9_-] chars with _) washes the name, then joins with : into sanitize(serverName)+\":\"+sanitize(toolName). So github's search → github:search, jira's search → jira:search, no collision. This prefixed \"scoped name\"'s finest destination is session/tools.ts: when MCP tools merge into the session tool set, the permission gate writes ctx.ask({ permission: key }) — this key is exactly the scoped name. So you can write in config \"github:* all allowed, jira:delete_* must ask,\" precisely governing each external server and tool. Naming both solves collisions and directly becomes the permission system's handle."},
            },
            {
